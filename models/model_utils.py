...
class MLP_Res(nn.Module):
    ...
    def forward(self, x):
        shortcut = self.conv_shortcut(x)  # (B, out_dim, N)
...
def sample_and_group(xyz, points, npoint, nsample, radius, use_xyz=True):
    ...
    Returns:
        new_xyz: (B, 3, npoint)
        new_points: (B, 3 | f+3 | f, npoint, nsample)
        idx_local: (B, npoint, nsample)
...
def sample_and_group_all(xyz, points, use_xyz=True):
    ...
    Returns:
        new_xyz: (B, 3, 1)
        new_points: (B, f|f+3|3, 1, nsample)
        idx: (B, 1, nsample)
        grouped_xyz: (B, 3, 1, nsample)
    """
...
class PointNet_FP_Module(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        Args:
            xyz1: (B, 3, N)
            xyz2: (B, 3, M)
            points1: (B, in_channel, N)
            points2: (B, in_channel, M)
        Returns:
...
def get_nearest_index(target, source, k=1, return_dis=False):
    ...
    Args:
        ...
        source: (B, 3, v2)
    Returns:
        nearest_index: (B, v1, k)
    """
    inner = torch.bmm(target.transpose(1, 2), source)  # (B, v1, v2)
    s_norm_2 = torch.sum(source**2, dim=1)  # (B, v2)
    t_norm_2 = torch.sum(target**2, dim=1)  # (B, v1)
    d_norm_2 = s_norm_2.unsqueeze(1) + t_norm_2.unsqueeze(2) - 2 * inner  # (B, v1, v2)
...
def indexing_neighbor(x, index):
    ...
    Args:
        x: (B, dim, num_points0)
        index: (B, num_points, k)
    Returns:
        feature: (B, dim, num_points, k)
    """
    ...
    feature = x[id_0, index]  # (B, num_points, k, num_dims)
    feature = feature.permute(0, 3, 1, 2).contiguous()  # (B, num_dims, num_points, k)
...
class SoftSplatCCM:
    """
    Soft Splatting for creating images from point clouds.
    Uses Gaussian-weighted splatting for smoother projections.
    """

    def __init__(self, TRANS, RESOLUTION, kernel_size=4, sigma=1.5):
        """
        Args:
            TRANS: Translation distance for camera views
            RESOLUTION: Output image resolution
            kernel_size: Size of the splatting kernel (1 or even number)
            sigma: Gaussian sigma for soft splatting weights
        """
        _views = np.asarray([
            [[0 * np.pi / 2, 0, np.pi / 2], [0, 0, TRANS]],
            [[1 * np.pi / 2, 0, np.pi / 2], [0, 0, TRANS]],
    ...
    def get_img(self, points):
        """Get depth image based on the prespecified specifications.
    ...
    def _soft_splat_depth(self, points, image_height, image_width):
        """Soft splatting for depth rendering with Gaussian weights."""
        epsilon = 1e-12
        device = points.device
        batch, num_points, _ = points.size()
        ...
        # Soft splatting with Gaussian weights
        if self.kernel_size == 1:
            # Simple nearest-neighbor (hard) splatting
            x_int = _x.long().clamp(0, image_height - 1)
            y_int = _y.long().clamp(0, image_width - 1)
            
            coords = x_int * image_width + y_int
            weight = 1.0 / (depth + epsilon)
            weighted_depth = depth * weight
            
            weight_img = torch.zeros(batch, image_height * image_width, device=device)
            value_img = torch.zeros(batch, image_height * image_width, device=device)
            
            weight_img.scatter_add_(1, coords, weight)
            value_img.scatter_add_(1, coords, weighted_depth)
            
            weight_img = weight_img.clamp(min=epsilon)
            depth_img = (value_img / weight_img).view(batch, image_height, image_width)
        else:
            # Gaussian soft splatting
            depth_img = self._gaussian_splat(
                _x, _y, depth, image_height, image_width, 
                self.kernel_size, self.sigma, device, batch
            )
        
        return depth_img

    def _gaussian_splat(self, _x, _y, values, H, W, kernel_size, sigma, device, batch):
        """Apply Gaussian soft splatting."""
        epsilon = 1e-12
        half_k = kernel_size // 2
        
        # Create kernel offsets
        offsets = torch.arange(-half_k, half_k, device=device).float()
        actual_k = offsets.shape[0]  # Use actual offset length
        
        # Expand coordinates for kernel
        x_exp = _x.unsqueeze(-1) + offsets.view(1, 1, -1)  # [B, N, actual_k]
        y_exp = _y.unsqueeze(-1) + offsets.view(1, 1, -1)  # [B, N, actual_k]
        
        # Create 2D grid
        x_grid = x_exp.unsqueeze(-1).expand(-1, -1, -1, actual_k)  # [B, N, actual_k, actual_k]
        y_grid = y_exp.unsqueeze(-2).expand(-1, -1, actual_k, -1)  # [B, N, actual_k, actual_k]
        
        # Compute Gaussian weights based on distance from original point
        dx = x_grid - _x.unsqueeze(-1).unsqueeze(-1)
        dy = y_grid - _y.unsqueeze(-1).unsqueeze(-1)
        dist_sq = dx ** 2 + dy ** 2
        gauss_weight = torch.exp(-dist_sq / (2 * sigma ** 2))
        
        # Depth-based weight (inverse depth)
        depth_weight = 1.0 / (values + epsilon)
        combined_weight = gauss_weight * depth_weight.unsqueeze(-1).unsqueeze(-1)
        
        # Mask valid coordinates
        x_int = x_grid.long()
        y_int = y_grid.long()
        valid_mask = (x_int >= 0) & (x_int < H) & (y_int >= 0) & (y_int < W) & (values.unsqueeze(-1).unsqueeze(-1) > 0)
        
        combined_weight = combined_weight * valid_mask.float()
        weighted_value = values.unsqueeze(-1).unsqueeze(-1) * combined_weight
        
        # Clamp coordinates for scatter
        x_int = x_int.clamp(0, H - 1)
        y_int = y_int.clamp(0, W - 1)
        coords = (x_int * W + y_int).view(batch, -1)
        
        combined_weight = combined_weight.view(batch, -1)
        weighted_value = weighted_value.view(batch, -1)
        
        weight_img = torch.zeros(batch, H * W, device=device)
        value_img = torch.zeros(batch, H * W, device=device)
        
        weight_img.scatter_add_(1, coords, combined_weight)
        value_img.scatter_add_(1, coords, weighted_value)
        
        weight_img = weight_img.clamp(min=epsilon)
        result = (value_img / weight_img).view(batch, H, W)
    ...
    def normalize_points_wo_scale(points):
        ...
        centroid = torch.mean(points, dim=1, keepdim=True)
        points = points - centroid
        max_distance = torch.max(torch.sqrt(torch.sum(points**2, dim=-1)), dim=1, keepdim=True)[0]
        max_distance = max_distance.unsqueeze(-1)
        points = points / (2 * max_distance + 1e-12)
        points = points + 0.5
        return points

    @staticmethod
    def point_transform_CCM(points, rot_mat, translation):
        """Transform points (with color) with rotation and translation."""
        rot_mat = rot_mat.to(points.device)
        translation = translation.to(points.device)
        temp_points = points[:, :, 0:3]
        temp_points = torch.matmul(temp_points, rot_mat)
    ...
    def get_CCM(self, points):
        """Get Color-Coded Map (CCM) with soft splatting.

        Args:
            points (torch.tensor): of size [B, N, 3]
        Returns:
            img (torch.tensor): of size [num_views, 3, RESOLUTION, RESOLUTION]
        """
        b, _, _ = points.shape
        v = self.translation.shape[0]

        # Normalize points to get colors
        colors = self.normalize_points_wo_scale(points)
        points_with_color = torch.cat([points, colors], dim=-1)

        _points = self.point_transform_CCM(
            points=torch.repeat_interleave(points_with_color, v, dim=0),
            rot_mat=self.rot_mat.repeat(b, 1, 1),
            translation=self.translation.repeat(b, 1, 1))

        img = self._soft_splat_ccm(
            points=_points,
            image_height=self.resolution,
            image_width=self.resolution,
        )
    ...
    def _soft_splat_ccm(self, points, image_height, image_width):
        """Soft splatting for CCM rendering with Gaussian weights."""
        epsilon = 1e-12
        device = points.device
        batch, num_points, _ = points.size()
        ...
        coord_y = (points[:, :, 1] / (points[:, :, 2] + epsilon))
        depth = points[:, :, 2]
        color = points[:, :, 3:6]  # RGB colors
        
        _x = ((coord_x + 1) * image_height) / 2
        _y = ((coord_y + 1) * image_width) / 2

        if self.kernel_size == 1:
            # Simple nearest-neighbor splatting
            x_int = _x.long().clamp(0, image_height - 1)
            y_int = _y.long().clamp(0, image_width - 1)
            
            coords = x_int * image_width + y_int
            weight = 1.0 / (depth + epsilon)
            weighted_color = color * weight.unsqueeze(-1)
            
            weight_img = torch.zeros(batch, image_height * image_width, device=device)
            color_img = torch.zeros(batch, image_height * image_width, 3, device=device)
            
            weight_img.scatter_add_(1, coords, weight)
            color_img.scatter_add_(1, coords.unsqueeze(-1).expand(-1, -1, 3), weighted_color)
            
            weight_img = weight_img.clamp(min=epsilon)
            ccm_img = (color_img / weight_img.unsqueeze(-1)).view(batch, image_height, image_width, 3)
            ccm_img = ccm_img.permute(0, 3, 1, 2)  # [B, 3, H, W]
        else:
            # Gaussian soft splatting for CCM
            ccm_img = self._gaussian_splat_ccm(
                _x, _y, depth, color, image_height, image_width,
                self.kernel_size, self.sigma, device, batch
            )
        
        return ccm_img

    def _gaussian_splat_ccm(self, _x, _y, depth, color, H, W, kernel_size, sigma, device, batch):
        """Apply Gaussian soft splatting for CCM."""
        epsilon = 1e-12
        half_k = kernel_size // 2
        num_points = _x.shape[1]
        
        # Create offset grid: for kernel_size=4, offsets=[-2,-1,0,1], actual_k=4
        offsets = torch.arange(-half_k, half_k, device=device).float()  # [actual_k]
        actual_k = offsets.shape[0]
        
        # Expand coordinates for kernel neighborhood
        # _x, _y: [B, N] -> x_exp, y_exp: [B, N, actual_k]
        x_exp = _x.unsqueeze(-1) + offsets.view(1, 1, -1)  # [B, N, actual_k]
        y_exp = _y.unsqueeze(-1) + offsets.view(1, 1, -1)  # [B, N, actual_k]
        
        # Create 2D kernel grid
        # x_grid: [B, N, actual_k, actual_k], y_grid: [B, N, actual_k, actual_k]
        x_grid = x_exp.unsqueeze(-1).expand(-1, -1, -1, actual_k)  # [B, N, actual_k, actual_k]
        y_grid = y_exp.unsqueeze(-2).expand(-1, -1, actual_k, -1)  # [B, N, actual_k, actual_k]
        
        # Compute Gaussian weights
        # _x: [B, N] -> [B, N, 1, 1]
        x_center = _x.unsqueeze(-1).unsqueeze(-1)  # [B, N, 1, 1]
        y_center = _y.unsqueeze(-1).unsqueeze(-1)  # [B, N, 1, 1]
        
        dx = x_grid - x_center  # [B, N, actual_k, actual_k]
        dy = y_grid - y_center  # [B, N, actual_k, actual_k]
        dist_sq = dx ** 2 + dy ** 2  # [B, N, actual_k, actual_k]
        gauss_weight = torch.exp(-dist_sq / (2 * sigma ** 2))  # [B, N, actual_k, actual_k]
        
        # Depth-based weight
        depth_weight = 1.0 / (depth + epsilon)  # [B, N]
        combined_weight = gauss_weight * depth_weight.unsqueeze(-1).unsqueeze(-1)  # [B, N, actual_k, actual_k]
        
        # Validity mask
        x_int = x_grid.long()  # [B, N, actual_k, actual_k]
        y_int = y_grid.long()  # [B, N, actual_k, actual_k]
        valid_mask = (x_int >= 0) & (x_int < H) & (y_int >= 0) & (y_int < W) & (depth.unsqueeze(-1).unsqueeze(-1) > 0)
        
        combined_weight = combined_weight * valid_mask.float()  # [B, N, actual_k, actual_k]
        
        # Expand for RGB channels: color [B, N, 3] -> [B, N, 1, 1, 3]
        weighted_color = color.unsqueeze(-2).unsqueeze(-2) * combined_weight.unsqueeze(-1)  # [B, N, actual_k, actual_k, 3]
        
        # Clamp coordinates and flatten
        x_int = x_int.clamp(0, H - 1)
        y_int = y_int.clamp(0, W - 1)
        coords = (x_int * W + y_int).view(batch, -1)  # [B, N*actual_k*actual_k]
        
        combined_weight = combined_weight.view(batch, -1)  # [B, N*actual_k*actual_k]
        weighted_color = weighted_color.view(batch, -1, 3)  # [B, N*actual_k*actual_k, 3]
        
        # Scatter add
        weight_img = torch.zeros(batch, H * W, device=device)
        color_img = torch.zeros(batch, H * W, 3, device=device)
        
        weight_img.scatter_add_(1, coords, combined_weight)
        color_img.scatter_add_(1, coords.unsqueeze(-1).expand(-1, -1, 3), weighted_color)
        
        weight_img = weight_img.clamp(min=epsilon)
        result = (color_img / weight_img.unsqueeze(-1)).view(batch, H, W, 3)
        result = result.permute(0, 3, 1, 2)  # [B, 3, H, W]
