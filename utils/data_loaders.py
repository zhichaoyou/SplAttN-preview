...
class ShapeNet34DataLoader(object):
    """ShapeNet34 split file loader.

    This project uses the ShapeNet55-style dataset wrapper (only loads complete
    point clouds and generates partial online in train/test loops). ShapeNet34
    differs only by the split list directory.
    """
    ...
    def _get_file_list(self, cfg, subset):
        # Load the dataset indexing file
        with open(
                os.path.join(cfg.DATASETS.SHAPENET34.CATEGORY_FILE_PATH,
                             subset + '.txt'), 'r') as f:
            lines = f.readlines()
        ...
        for line in lines:
            ...
            file_list.append({
                ...
                'model_id': model_id,
                'gtcloud_path': cfg.DATASETS.SHAPENET34.COMPLETE_POINTS_PATH % (line),
            })
...
class ShapeNetUnseen21DataLoader(object):
    """ShapeNet-Unseen21 split file loader.

    This project uses the ShapeNet55-style dataset wrapper (only loads complete
    point clouds and generates partial online in train/test loops). ShapeNet-Unseen21
    differs only by the split list directory.
    """
...
DATASET_LOADER_MAPPING = {
    ...
    'ShapeNet34': ShapeNet34DataLoader,
    'ShapeNet-Unseen21': ShapeNetUnseen21DataLoader
}  # yapf: disable
