...
def train_net(args, cfg):
    ...
    # FP32 mode, no need to load scaler

        warmup_epochs = min(cfg.TRAIN.WARMUP_STEPS, max(1, cfg.TRAIN.N_EPOCHS - 1))
        steps = warmup_epochs + 1
    ...
    for epoch_idx in range(init_epoch + 1, cfg.TRAIN.N_EPOCHS + 1):
        ...
        with tqdm(train_data_loader, disable=args.local_rank != 0) as t:
            for batch_idx, (taxonomy_ids, model_ids, data) in enumerate(t):
                ...
                gt = data['gtcloud']
                batch_size, npoints, _ = gt.size()
                if batch_size % 2 != 0:
                    gt = torch.cat([gt, gt], dim=0)
                partial, _ = seprate_point_cloud(
                    gt,
                    npoints,
                ...
                # FP32 forward and backward pass
